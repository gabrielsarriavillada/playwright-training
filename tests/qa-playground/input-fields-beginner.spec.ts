...
test('Verify input placeholder disappears on typing', async({page}) => {
    await page.goto('https://www.qaplayground.com/practice/input-fields');
    await expect(page.getByTestId('input-movie-name')).toHaveAttribute('placeholder', 'Enter hollywood movie name');
    await expect(page.locator('[data-testid=input-movie-name]:placeholder-shown')).toBeVisible();
    await page.getByTestId('input-movie-name').fill('Die hard');
    await expect(page.locator('[data-testid=input-movie-name]:placeholder-shown')).toBeHidden();
});
...
test('Verify appended text value is retained in the field', async({page}) => {
    ...
    await expect(page.getByTestId('input-append-text')).toHaveValue('I am good appended text');
});
...
test('Verify getAttribute return the correct input value', async({page}) => {
    await page.goto('https://www.qaplayground.com/practice/input-fields');
    await expect(page.getByTestId('input-verify-text')).toHaveAttribute('value', 'QA PlayGround');
...
test('Verify readonly input field does not accept user typing', async({page}) => {
    ...
    await expect(page.getByTestId('input-readonly')).toHaveValue('This text is readonly');
});
